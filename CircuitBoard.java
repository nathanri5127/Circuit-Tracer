...
public class CircuitBoard {
	...
	public CircuitBoard(String filename) throws FileNotFoundException {
		...
		if (line.length != 2) {
			throw new InvalidFileFormatException("Invalid board size");
		}
		try { 
		...
		for (int r=0; r<ROWS; r++) {
        	...
        	for (int c = 0; c < COLS; c++) {
        		...
        		if (startCount >= 2 || endCount >= 2) {
        			lineScan.close();
        			throw new InvalidFileFormatException("Invalid start or end points");
        		}
        		if (ALLOWED_CHARS.indexOf(check) != -1) {
        			board[r][c] = check;
        		} else {
        			lineScan.close();
        			throw new InvalidFileFormatException("Invalid board");
        		}
        	}
        	if (lineScan.hasNext()) {
        		lineScan.close();
		...
		if ((startCount == 0) || (endCount == 0)) {
			throw new InvalidFileFormatException("Missing start or end point");
		}
        if (fileScan.hasNext() ) {
        	if (!fileScan.next().equals("")) {
        		throw new InvalidFileFormatException("Invalid array size");
        	}
        }
		if (fileScan != null) {
            fileScan.close();
